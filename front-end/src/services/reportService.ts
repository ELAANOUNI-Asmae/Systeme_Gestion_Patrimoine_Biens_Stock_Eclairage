...
const buildAssetsReport = async (params: GenerateReportParams): Promise<ReportData> => {
  const arabic = params.language === "ar";
  const activeAssets = (await bienService.getAll()) as unknown[];
  const bienApi = bienService as unknown as { getArchived?: () => Promise<unknown[]> };
  const archivedAssets = bienApi.getArchived ? await bienApi.getArchived() : [];
