...
class _StockScreenState extends State<StockScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<AppState>().loadStockData();
      }
    });
  }
  ...
  Widget build(
    ...
    return Column(
      children: [
        if (state.stockLoading) const LinearProgressIndicator(minHeight: 2),
        if (state.stockError != null && !state.stockLoading)
          Material(
            color: Theme.of(context).colorScheme.errorContainer,
            child: ListTile(
              dense: true,
              title: Text(context.tr('Impossible de charger le stock.', 'تعذر تحميل المخزون.')),
              trailing: IconButton(
                tooltip: context.tr('Réessayer', 'إعادة المحاولة'),
                onPressed: state.loadStockData,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (alert.status == RestockAlertStatus.readyNotified)
                        ...
                        Permissions.approveItemRequest,
                      ))
                        FilledButton.tonalIcon(
                          onPressed: enough
                              ? () async {
                                  try {
                                    await state.notifyRestockReady(
                                      alert.id,
                                    );
              ...
              children: [
                ...
                if (request.status == SupplyRequestStatus.pending &&
                  ...
                  IconButton.filledTonal(
                    ...
                    onPressed: () async {
                      try {
                        await state.setSupplyRequestStatus(
                          request.id,
                ...
                if (request.status == SupplyRequestStatus.approved &&
                  ...
                  FilledButton.icon(
                    onPressed: () async {
                      try {
                        await state.confirmSupplyRequestReceived(
                          request.id,
                        );
        ...
        builder: (
          ...
          children: [
            ...
            DocumentEditor(
              ...
              onChanged: (documents) async {
                article.documents = documents;
                try {
                  await state.saveArticle(article);
                } on StateError catch (error) {
                  if (sheetContext.mounted) _stateError(sheetContext, error);
                }
              },
              remoteLoader: state.loadStockDocument,
            ),
            const SizedBox(
              height: 12,
            ),
            Wrap(
              ...
              children: [
                ...
                if (state.hasPermission(
                  Permissions.updateItemQuantity,
                ))
                  FilledButton.tonalIcon(
                    onPressed: () => _movementForm(
                      context,
                  ...
                  OutlinedButton.icon(
                    onPressed: () async {
                      ...
                      if (ok && context.mounted) {
                        try {
                          await state.deleteArticle(article.id);
                        } on StateError catch (error) {
                          if (sheetContext.mounted) _stateError(sheetContext, error);
                          return;
                        }
                  ...
                  children: [
                              ...
                              children: [
                                ...
                                FilledButton.tonalIcon(
                                  onPressed: () async {
                                    ...
                                    try {
                                      await state.createRestockAlert(
                    ...
                    FilledButton(
                      onPressed: () async {
                        ...
                        try {
                          await state.createSupplyRequest(
                            articleId: selected!.id,
                            quantity: requested,
                            reason: reason.text.trim(),
                            documents: documents,
